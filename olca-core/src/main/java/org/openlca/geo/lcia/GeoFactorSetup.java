...
public class GeoFactorSetup {
	 ...
	 */
	public final List<GeoProperty> properties = new ArrayList<>();
	 ...
	 * generated.
	 */
	public static GeoFactorSetup read(File file, IDatabase db) {
		if (file == null)
			return null;
		...
		var setup = new GeoFactorSetup(features);
		setup.properties.addAll(GeoProperty.collectFrom(features));
		return setup;
	}

	private static GeoFactorSetup fromSerialized(JsonObject json, IDatabase db) {
