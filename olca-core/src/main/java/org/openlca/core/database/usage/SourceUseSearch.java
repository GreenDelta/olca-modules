...
public class SourceUseSearch extends BaseUseSearch<SourceDescriptor> {
	...
	public List<CategorizedDescriptor> findUses(Set<Long> ids) {
		...
		processDocIds.addAll(queryForIds("id", "tbl_process_docs", ids,
				"f_publication"));
		processDocIds.addAll(queryForIds("f_process_doc",
				"tbl_process_sources", ids, "f_source"));
		return queryFor(ModelType.PROCESS, processDocIds, "f_process_doc");
	}
}
