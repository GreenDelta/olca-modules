...
public class Upgrade3 implements IUpgrade {
	...
	private void createDQScoreTable() throws Exception {
		util.createTable("tbl_dq_scores", "CREATE TABLE tbl_dq_scores ( "
				...
				+ "description CLOB(64 K), " + "label VARCHAR(255), "
				+ "uncertainty DOUBLE default 0, "
				+ "f_dq_indicator BIGINT, " + "PRIMARY KEY (id)) ");
	}

	private void modifyProcessTable() throws Exception {
		util.createColumn("tbl_processes", "dq_entry",
				"dq_entry VARCHAR(50)");
		util.createColumn("tbl_processes", "f_dq_system",
				"f_dq_system BIGINT");
		util.createColumn("tbl_processes", "f_exchange_dq_system",
				"f_exchange_dq_system BIGINT");
		util.createColumn("tbl_processes", "f_social_dq_system",
				"f_social_dq_system BIGINT");
	}

	private void modifyExchangeTable() throws Exception {
		util.renameColumn("tbl_exchanges", "pedigree_uncertainty", "dq_entry",
				"VARCHAR(50)");
	}

	private void modifyProductSystemTable() throws Exception {
		util.createColumn("tbl_product_systems", "cutoff",
				"cutoff DOUBLE");
	}
