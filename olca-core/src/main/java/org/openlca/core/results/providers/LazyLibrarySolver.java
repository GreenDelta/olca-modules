...
public class LazyLibrarySolver implements ResultProvider {
	...
	public double[] totalRequirements() {
		...
		for (int i = 0; i < techF.columns(); i++) {
			...
			if (libs.dataPackages.isLibrary(product.dataPackage()))
				continue;
	...
	public double[] techColumnOf(int techFlow) {
		...
		if (!libs.dataPackages.isLibrary(packageId)) {
	...
	public double[] solutionOfOne(int techFlow) {
		...
		if (libs.dataPackages.isLibrary(start.dataPackage())) {
			...
			for (int i = 0; i < sf.length; i++) {
				...
				if (libs.dataPackages.isLibrary(provider.dataPackage())) {
	...
	public double[] unscaledFlowsOf(int techFlow) {
		...
		// exactly the first part of the combined index
		if (!libs.dataPackages.isLibrary(libId)) {
