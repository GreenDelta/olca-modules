 ...
 */
public final class ProcessLinkSearchMap {

	private TLongObjectHashMap<TIntArrayList> providerIndex;
	private TLongObjectHashMap<TIntArrayList> recipientIndex;
	private ArrayList<ProcessLink> data;
	...
	public ProcessLinkSearchMap(Collection<ProcessLink> links) {
		...
		for (int i = 0; i < data.size(); i++) {
			...
			index(link.recipientId, i, recipientIndex);
		}
	}
	...
	private List<ProcessLink> getLinks(TIntIterator iterator) {
		...
		while (iterator.hasNext())
			links.add(data.get(iterator.next()));
		return links;
	}
}
