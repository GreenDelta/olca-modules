...
public class InMemoryProtoStore implements ProtoStoreReader, ProtoStoreWriter {
	...
	public ProtoEpd getEpd(String id) {
		return get(ModelType.EPD, ProtoEpd.class, id);
	}

	@Override
	public ProtoResult getResult(String id) {
		return get(ModelType.RESULT, ProtoResult.class, id);
	}
	...
	private <T extends AbstractMessage> T get(
		...
		return protoType.isInstance(proto)
	...
	@Override
	public void putEpd(ProtoEpd epd) {
		if (epd != null) {
			put(ModelType.EPD, epd, epd.getId());
		}
	}

	@Override
	public void putResult(ProtoResult result) {
		if (result != null) {
			put(ModelType.RESULT, result, result.getId());
		}
	}
