...
public class ZipStore implements ProtoStoreReader, AutoCloseable {
	...
	public ProtoResult getResult(String id) {
		var proto = readBin("results", id, ProtoResult.parser());
		if (proto != null)
			return proto;
		var builder = ProtoResult.newBuilder();
		return readJson("results", id, builder)
			? builder.build()
			: null;
	}
	...
	public ProtoEpd getEpd(String id) {
		var proto = readBin("epds", id, ProtoEpd.parser());
		if (proto != null)
			return proto;
		var builder = ProtoEpd.newBuilder();
		return readJson("epds", id, builder)
			? builder.build()
			: null;
	}
