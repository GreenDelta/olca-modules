...
class ImportCache {
	...
	private <T extends RefEntity> boolean skipImport(T model, ProtoWrap<?> proto) {
	...
	private ProtoWrap<?> readProto(ModelType type, String refId) {
		if (type == null)
			return null;
		return switch (type) {
			case PROJECT -> {
				var proto = imp.reader.getProject(refId);
				yield proto != null ? ProtoWrap.of(proto) : null;
			}
			case IMPACT_METHOD -> {
