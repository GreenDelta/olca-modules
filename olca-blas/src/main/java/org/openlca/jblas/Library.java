...
/**
 * A helper class for loading the native JBLAS libs from a file directory.
 */
public class Library {
	...
	public static void loadFromDir(File dir) {
		if (loaded) {
			log.trace("jblas lib already loaded");
			return;
		}
		File realDir = makeRealDir(dir);
		log.trace("try load jBlas from directory {}", realDir);
		String jblasLib = System.mapLibraryName("jblas");
		try {
			String blasPath = getJarPath(jblasLib);
			loaded = loadLib(realDir, jblasLib, blasPath);
			checkLib();
		} catch (Throwable e) {
	...
	private static File makeRealDir(File dir) {
		...
		String subPath = "jblas_".concat(VERSION).concat(sep).concat(os)
				.concat(sep).concat(arch);
	...
	private static boolean copyLib(String jarPath, File file)
			throws IOException {
		InputStream is = Library.class.getResourceAsStream(jarPath);
		if (is == null)
			return false;
