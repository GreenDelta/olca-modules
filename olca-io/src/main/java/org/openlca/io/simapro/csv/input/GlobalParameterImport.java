 ...
 */
class GlobalParameterImport {

	private Logger log = LoggerFactory.getLogger(getClass());

	private List<InputParameterRow> inputParams = new ArrayList<>();
	private List<CalculatedParameterRow> calculatedParams = new ArrayList<>();
	private ParameterDao dao;

	public GlobalParameterImport(IDatabase database,
			List<InputParameterRow> inputParams,
			List<CalculatedParameterRow> calculatedParams) {
	...
	public void run() {
		...
		for (InputParameterRow row : inputParams) {
		...
		for (CalculatedParameterRow row : calculatedParams) {
			if (contains(row.getName(), globals))
				continue;
